...
<section class="max-w-11xl mx-auto mt-1 px-4 sm:px-6 lg:px-8">
  ...
  <form id="SettingForm" action="{{ route('authenticateAndSave') }}" class="p-4 md:p-5 bg-gray-200 rounded-lg shadow" method="POST">
    <div class="grid gap-4 mb-4 lg:grid-cols-3 md:grid-cols-2">
      ...
      <div>
        <label for="fullfilment" class="block mb-2 text-sm font-medium text-gray-900">Fullfillment Location</label>
        <select id="fullfilment" name="fullfilment"
          ...
          @foreach(['islamabad','lahore','faislabad','rawalpindi','karachi'] as $city)
            <option value="{{ $city }}" {{ ($settings->fulfillment_location ?? '') === $city ? 'selected' : '' }}>
              {{ ucfirst($city) }}
            </option>
          @endforeach
        </select>
      ...
      <div>
        ...
        <select id="accounttype" name="account_type"
          ...
          <option value="live" {{ ($settings->account_type ?? '') === 'live' ? 'selected' : '' }}>Live</option>
          <option value="offline" {{ ($settings->account_type ?? '') === 'offline' ? 'selected' : '' }}>Offline</option>
        </select>
      </div>

      <div>
        <label for="auto_push_orders" class="block mb-2 text-sm font-medium text-gray-900">Order Push Automatically on CMS</label>
      ...
      <div>
        ...
        <input type="number" id="price" name="price"
          class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
          placeholder="$2999"
          value="{{ old('price', $settings->price ?? '') }}">
      </div>
